...
## 配套官网的 Live Demo 依赖哪个模型？

结论是：**[modly3d.app](https://modly3d.app/) 本身不运行某个图生 3D 模型；它是产品介绍、文档、下载入口和扩展目录。**首页的 “Live Demo” 加载的是已经生成好的 `cat.glb`，浏览器只负责显示和交互旋转，不负责推理。

本轮通过真实浏览器检查首页网络活动，得到的证据是：

- 页面没有图片上传框或在线“生成”操作。
- 3D 展示下载 `cat.png` 和静态 `cat.glb`，由 Google `<model-viewer>` 组件渲染。
- 没有下载 `.safetensors`、GGUF、ONNX 等模型权重，也没有调用 3D 生成 / 推理接口。
- 唯一额外 API 请求属于网站事件统计，不是生成任务。
- `cat.glb` 的 glTF 元数据只记录 `https://github.com/mikedh/trimesh` 为导出器，没有记录 Hunyuan、TripoSG 或 TRELLIS 模型名。

因此需要区分三个问题：

| 问题 | 结论 |
|---|---|
| 官网网页运行哪个模型？ | 不运行模型；它展示预生成 GLB。 |
| 首页猫模型由哪个模型生成？ | 官方未披露，当前公开页面和 GLB 元数据都无法确定。 |
| 下载 Modly 后实际依赖哪个模型？ | 取决于用户安装并在工作流中选择的扩展，不是单一固定模型。 |

官网写明主推 **Hunyuan3D 2、TripoSG、Trellis2 + 用户扩展**，同时明确流程是“选择已安装扩展 → 在本地 NVIDIA GPU 生成”。[官方扩展目录](https://modly3d.app/extensions)还要求先从 GitHub 安装扩展，再下载对应模型或 variant。这说明桌面安装包不是一个已经内置全部权重、打开即用的在线模型服务。

**Hunyuan3D 2 Mini 可以视为文档和低门槛部署中的常见入门扩展，但不能据此推断官网猫模型就是它生成的。**官网样例没有披露模型、参数、耗时、显存和后处理过程，所以它只能证明展示效果，不能作为某个模型的可复现实测证据。

当前目录口径也需要注意：截至 2026-08-27，[官网扩展目录](https://modly3d.app/extensions)显示 24 个已发布扩展，既有 model，也有 process，并包含社区提交；“在官方目录中”不等于“安装包内置”，也不等于“全部由 Modly 官方团队维护”。
