...
| [cangjie-skill](https://github.com/kangarooking/cangjie-skill) | 把书籍、课程、访谈、播客等长内容转成可触发、可执行、可测试的 Agent Skills；本地已完成《左耳听风》代理语料受控案例 | `archived` | `published` | [研究结论](studies/cangjie-skill/README.md) · [Web 页面](https://yydshly.github.io/0827_githubcode_study/cangjie-skill/) |
...
**官网演示的准确口径：** `modly3d.app` 不在网页中运行图生 3D 模型。首页只加载预先生成的 `cat.glb` 并用浏览器 3D 组件展示；官方没有披露该样例由 Hunyuan、TripoSG 还是 TRELLIS 生成。真正推理发生在下载后的 Modly 桌面端，用户需要安装扩展、下载权重并选择模型；因此不存在一个固定的“官网后端模型”。
